...
ProgressBar::ProgressBar(Widget* parent) :
  Widget(parent), _maximum(100), _minimum(0), _value(0), _orientation(
      Horizontal)
{
...
{
  Size s = designer()->sizeHint(ProgressBarSizeHint);
...
void
ProgressBar::compose(const Rectangle& rect)
{
  Painter p(this);
  p.begin(rect);
  designer()->drawProgressBar(&p, this);
  p.end();
}
